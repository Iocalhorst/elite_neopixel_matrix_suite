    ...
    if ((dx*dx) > (dy*dy)) {
        ...
        do {
            ...
            if ((x>=0)&&(y>=0)&&(x<10)&&(y<30)) {
                if (ls->illumination_map[x][y]<0) return;
                ls->illumination_map[x][y]=1;
...
void line_of_sight_demo_cast_illumination_map(line_of_sight_demo_t* self,light_source_t* ls){
    ...
    for (int cursor_x=0;cursor_x<10;cursor_x++) {light_source_cast_ray(ls,cursor_x,0);};
    ...
    for (int cursor_y=0;cursor_y<30;cursor_y++) {light_source_cast_ray(ls,0,cursor_y);};
...
/*  if ((self->light_source.fx+fdx>=(float)ente->screen_width)||(self->light_source.fx+fdx<=0.0f)) {
    self->light_source.fvx*=-1.0f;
  }else {self->light_source.fx+=fdx;};

  if ((self->light_source.fy+fdy>=(float)ente->screen_height)||(self->light_source.fy+fdy<=0.0f)) {
    self->light_source.fvy*=-1.0f;
  }else {self->light_source.fy+=fdy;};
*/
bool bounce=false;
  if (self->light_source.fx+fdx>(float)ente->screen_width-1.0f){
    self->light_source.fvx=0.0f;
    self->light_source.fvy=7.5f;
    bounce=true;
...
line_of_sight_demo_cast_illumination_map(self,&self->light_source);
  ...
  for (int y=0;y<30;y++){
    for (int x=0;x<10;x++){
            if (self->map[x][y]>0) {
                sfRGBA c={64.0f,96.0f,255.0f,255.0};
                elite_pixel_game_fputpixelRGBA(ente,x,y,c);
            }
        }
    }

  elite_pixel_game_draw_filled_circle(ente,self->light_source.fx,self->light_source.fy,1.5f,self->light_source.core_color);
  ...
  for (int y=0;y<30;y++){
    for (int x=0;x<10;x++){
            if (self->light_source.illumination_map[x][y]>0) {
                ...
                sfRGBA c_local;
                c_local.fr=self->light_source.light_color.fr;
                c_local.fg=self->light_source.light_color.fg;
                c_local.fb=self->light_source.light_color.fb;
                c_local.fa=self->light_source.light_color.fa;
        ...
        /*    sfRGBA c_local;
            ...
            c_local.fg=16.0f;
            c_local.fg=64.0f;*/
                elite_pixel_game_fputpixelRGBA(ente,x,y,c_local);
                };
            };
    }
  }
