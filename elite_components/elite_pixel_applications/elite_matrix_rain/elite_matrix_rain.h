...
typedef struct {
  ...
  float fx;
  float fy;
  m_streamer_t m_streamer[NUM_STREAMERS];
  float change_intervall_seconds;
...
//  };
  ...
  for (int i=0;i<NUM_STREAMERS;i++) {
    self->m_streamer[i].fx=esp_random()%10;
    self->m_streamer[i].fy=0.0f-esp_random()%30;
  };
  self->change_intervall_seconds=0.2f;
  self->time_until_change=self->change_intervall_seconds;
...
static sfRGBA col_matrix={16.0f,192.0f,8.0f,255.0f};
static sfRGBA col_cursor={16.0f,255.0f,8.0f,255.0f};
...
bool elite_matrix_rain_on_user_update(void* params,elite_pixel_game_t *ente,float fElapsedTime){
  ...
  //update streamers position

    for (int i=0;i<NUM_STREAMERS;i++) {
      self->m_streamer[i].fy+=(fElapsedTime*20.0f);
      if (self->m_streamer[i].fy>=30.0f) {
        self->m_streamer[i].fy-=(30.0f+(esp_random()%15));
        self->m_streamer[i].fx=esp_random()%10;
      };
    };
...
//turn the lights down....streamer traces
  ...
  for (int i=0;i<NUM_STREAMERS;i++) {
      int shader_y=(int)(self->m_streamer[i].fy/(30/NUM_ROWS));
      int shader_x=(int)(self->m_streamer[i].fx/(10/NUM_COLS));
      if (shader_x>=0&&shader_x<NUM_COLS&&shader_y>=0&&shader_y<NUM_ROWS) {
  ...
  //shader_x=2;
  self->time_until_change-=fElapsedTime;
  if (self->time_until_change<=0.0f) {
      int idx_change=esp_random()%(NUM_ROWS*NUM_COLS);
      self->m_chars[idx_change]=esp_random()%NUM_GLYPHS;
      self->time_until_change=self->change_intervall_seconds;
  };
  ...
  for (int i_row=0;i_row<NUM_ROWS;i_row++){
    for (int i_col=0;i_col<NUM_COLS;i_col++) {
        ...
        col_local.fa*=self->m_floats[idx];
        col_local.fr*=self->m_floats[idx];
        col_local.fg*=self->m_floats[idx];
        col_local.fb*=self->m_floats[idx];
        if (self->m_floats[idx]==1.0f) {col_local=col_cursor;};
      ...
      //  if ((i_row==shader_y)&&(i_col==shader_x)) {col_local=col_cursor;};
        int canvas_x=i_col*(10/NUM_COLS);
        int canvas_y=i_row*(30/NUM_ROWS);
...
elite_pixel_game_config_t pixel_game_config2={
...
void elite_matrix_rain_start_task(){
    ...
    xTaskCreate(&elite_pixel_game_task, "elite_pixel_game_task", 4096,&pixel_game_config2, 5, NULL);
    //ELOG("DEBUG : [elite_matrix_rain_start_task] exit\n");
